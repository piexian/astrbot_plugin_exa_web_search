...
- Agent 任务结束后主动返回任务号和正文，超长结果自动发送以任务号命名的 Markdown 文件。
- 新增通知持久化、重试与重启续发；文件不受支持时分段返回全文，发送结束后清理临时文件，送达后释放快照。
- 修复 `stats` 摘要静默截断，提示全文导出方式；普通查询不自动发文件，仅 `stats -q` 显式导出。
- 修复归档清理预览与容量检查，保护待通知任务，并在通知投递结束后重新检查容量。
